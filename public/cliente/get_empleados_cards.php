...
try {
    ...
    foreach ($empleados as $emp) {
        // Manejar correctamente la foto que podría ser un recurso o BLOB
        if (isset($emp['foto_de_perfil']) && is_resource($emp['foto_de_perfil'])) {
            // Si es un recurso, generamos una URL de imagen en base64
            $foto_data = stream_get_contents($emp['foto_de_perfil']);
            $foto = 'data:image/jpeg;base64,' . base64_encode($foto_data);
        } else if (!empty($emp['foto_de_perfil']) && is_string($emp['foto_de_perfil'])) {
            // Si es una cadena (podría ser una URL o un path)
            $foto = htmlspecialchars($emp['foto_de_perfil']);
        } else {
            // Imagen por defecto
        ...
        if (!empty($emp['especialidades'])) {
            ...
            foreach ($especialidades as $esp) {
                echo '<span class="employee-specialty px-2 py-1 bg-gray-100 text-gray-700 rounded-full text-xs">' . htmlspecialchars(trim($esp)) . '</span>';
            }
            echo '</div>';
        }
